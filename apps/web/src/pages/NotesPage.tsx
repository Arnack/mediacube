...
export function NotesPage() {
  ...
  async function handleAddUrl(e: React.FormEvent) {
    ...
    try {
      ...
      setUrlInput('')
      toast({ title: 'Article saved', description: note.title })
      load()
    } catch (err: any) {
      toast({ title: 'Failed to parse URL', description: err.message, variant: 'destructive' })
    } finally {
      setAddingUrl(false)
    }
  }
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <form onSubmit={handleAddUrl} className="border-b px-4 py-2 flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Paste article URL to save & summarize…"
          ...
          <div className="divide-y">
            {loading ? (
              ...
              <div className="p-8 text-center">
                ...
                <Button size="sm" className="mt-3" onClick={() => nav('/notes/new')}>Create your first note</Button>
              </div>
            ) : (
              notesList.map(note => (
                <button
      ...
      {showSuggestions && (
        <div className="w-72 border-l flex flex-col">
          <div className="border-b px-4 py-3 flex items-center justify-between">
            <span className="text-sm font-medium flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5" /> AI Suggestions</span>
